...
const TwoStepsV1 = () => {
  ...
  const onSubmit = async (data: any) => {
    let payload = {
      employeeEmail: query?.email,
      otp: data?.val1 + data?.val2 + data?.val3 + data?.val4
    }
    const res = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/verifydOtp`, payload, {
      headers: {
  ...
  const resendOtp = async (e: any) => {
    ...
    const res = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/resendOtp`, payload, {
      headers: {
        'Access-Control-Allow-Origin': '*',
        'Content-Type': 'application/json'
      }
        ...
        <Card>
          <CardContent sx={{ p: theme => `${theme.spacing(10.5, 8, 8)} !important` }}>
            ...
            <Box sx={{ mt: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <LinkStyled href='/' onClick={e => resendOtp(e)}>
                Resend
              </LinkStyled>
            </Box>
          </CardContent>
